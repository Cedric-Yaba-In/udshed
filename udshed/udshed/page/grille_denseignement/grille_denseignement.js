frappe.pages['grille-denseignement'].on_page_load = function(wrapper) {
    ...
    frappe.teaching_grid = new TeachingGrid(wrapper);
...
class TeachingGrid {
    constructor(wrapper) {
        ...
        this.current_classe = 'BTS_GL1';
        this.current_academic_year = '2024-2025';
        this.datatable = null;
		this.data= []
    ...
    setup_page() {
        ...
        // Boutons d'action
        this.page.set_primary_action(__('Importer'), function() {
            me.show_import_dialog();
        }, 'upload');
        
        this.page.add_menu_item(__('Exporter'), function() {
            me.export_grid();
        }, 'download');
        
        this.page.add_menu_item(__('Rafraîchir'), function() {
            me.refresh();
        }, 'refresh');
    }
    ...
    setup_filters() {
        ...
        me.page.add_field({
            ...
            label: __('Classe'),
            fieldtype: 'Select',
            options: ['BTS_GL1', 'BTS_GL2', 'BTS_GL3'],
            default: 'BTS_GL1',
            change: function() {
                me.current_classe = this.get_value();
                me.refresh();
            }
        });
        ...
        me.page.add_field({
            ...
            fieldtype: 'Select',
            options: ['2024-2025', '2023-2024', '2022-2025'],
            default: '2024-2025',
            change: function() {
                me.current_academic_year = this.get_value();
                me.refresh();
            }
        });
        ...
        me.page.add_field({
            ...
            default: '1',
            change: function() {
                me.current_semester = parseInt(this.get_value());
                me.load_data();
            }
        });
    }
    ...
    load_stats() {
        ...
        frappe.call({
            method: 'udshed.api.teaching_test.get_stats',
            args: {
                classe: me.current_classe,
                academic_year: me.current_academic_year
            },
            callback: function(r) {
                if (r.message) {
					this.data = r.message
                    $('#stat-ue').text(r.message.ue_count || 0);
                    $('#stat-courses').text(r.message.course_count || 0);
                    $('#stat-credits').text(r.message.total_credits || 0);
                    $('#stat-hours').text(r.message.total_hours || 0);
                }
            }
        });
    }
    ...
    load_data() {
        ...
        frappe.call({
            ...
            callback: function(r) {
                if (r.message) {
					console.log("Data grid to show",r.message)
                    me.data = r.message
                    me.render_datatable(r.message);
                    me.update_totals(r.message);
                } else {
    ...
    render_datatable() {
        ...
        data.forEach(function(ue) {
            ...
            ue.courses.forEach(function(course) {
                var total_p = (course.nombre_dheure_cm || 0) + (course.nombre_dheure_td || 0) + (course.nombre_dheure_tp || 0);
                var percentage = course.total_hours ? ((course.fait_hours || 0) / course.total_hours * 100).toFixed(1) + '%' : '0%';
                rows.push([
                    '',
                    course.code,
                    course.title,
        ...
        if (me.datatable) {
            ...
            me.datatable = new frappe.DataTable($('.table-area')[0],{
                ...
                getEditor: function(colIndex, rowIndex, value, column, row) {
					...
					console.error("Row data ",colIndex, rowIndex, value, column, row)
                ...
                get_data: function() {
                    return new Promise(resolve => {
                        frappe.call({
                            ...
                            callback: function(r) {
                                if (r.message && r.message.teachers) {
                                    resolve(r.message.teachers);
                                } else {
                                    resolve([]);
                                }
                            }
...
handleTeacherCellClick(rowIndex, colIndex) {
    ...
    frappe.call({
        ...
        callback: function(r) {
            if (r.message && r.message.name) {
                me.open_teacher_assignment_dialog(r.message.name, courseCode, rowIndex, colIndex);
            } else {
        ...
        $(displayDiv).on('click', function() {
           ...
           return {
                ...
                setValue: function(val) {
                    ...
                    // if (val && val.includes('||')) {
                    //     var teachers = val.split('||');
                    //     var displayText = teachers.join(', ');
                    //     if (teachers.length > 2) {
                    //         displayText = teachers[0] + ' + ' + (teachers.length - 1) + ' autres';
                    //     }
                    //     displayDiv.innerHTML = `<span class="multiple-teachers" title="${teachers.join('\n')}">
                    //         <i class="fa fa-users"></i> ${displayText}
                    //         <span class="badge">${teachers.length}</span>
                    //     </span>`;
                    // } else {
                    //     displayDiv.innerHTML = val || '<span class="text-muted">Cliquez pour assigner</span>';
                    // }
                }
            };
    ...
    update_totals() {
        var totals = { cm: 0, td: 0, tp: 0, total: 0 };
            ...
            fields: [
                {
                    ...
                    default: me.current_academic_year,
                    reqd: 1
                },
                {
                    fieldtype: 'Attach',
                    ...
                    callback: function(r) {
                        if (r.message && r.message.success) {
                            frappe.msgprint({
                                ...
                                message: `
                                    <p>UE créées: ${r.message.stats.ues_created || 0}</p>
                                    <p>Cours créés: ${r.message.stats.courses_created || 0}</p>
                                    <p>Cours mis à jour: ${r.message.stats.courses_updated || 0}</p>
                                `,
                                indicator: 'green'
                            });
                            me.refresh();
    ...
    export_grid() {
        ...
        frappe.call({
            method: 'udshed.api.teaching_test.export_grid',
            args: {
                classe: me.current_classe,
                academic_year: me.current_academic_year,
                semester: me.current_semester
                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <div class="panel-title">
                            Grille d'enseignement - Semestre <span id="semester-title">1</span>
                        </div>
                    </div>
                    <div class="panel-body">
                        <div class="table-area" style="min-height: 300px;">
                            <div class="text-center text-muted loading">
